...
r.sendEmail = async ({ json }) => {
	...
	const { customer_email, customer_name, order, lname, gross_price, gross_cur } = JSON.parse(json)
...
async function sendEmail(obj) { console.log("obj:" , obj)
	...
	const emailMessageHtml = await readContent(join("src", "client", "confirmationEmail.html"), "utf8").then(data => {
